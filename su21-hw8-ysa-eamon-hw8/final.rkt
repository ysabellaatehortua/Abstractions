...
(define (every-n n s)
  (cons (stream-first s) (every-n-helper n s)))

(define (every-n-helper n s)
  (cond [(<= (stream-length s) n) empty-stream]
        [(cons (stream-ref s n) (stream->list(every-n-helper n (stream-tail s n))))]
        ))
...
(define (has-at-least n s)
  (cond
    [(and (stream-empty? s) (> n 0)) #f]
    [(not (stream-empty? s)) (has-at-least (- n 1) (stream-rest s))]
    [else #t]))
...
(define (remove-sym-k sym lst k err-k)
  (cond [(empty? lst) (k '())]
        [(not (symbol? (first lst))) (err-k (first lst))]
        [(eq? sym (car lst)) (remove-sym-k sym (rest lst) k err-k)]
        [else (remove-sym-k sym (rest lst)
                            (lambda (x)
                              (k (cons (car lst) x)))err-k)]
        ))

; Please write Oberlin's Honor Pledge and write your name below.
; Honor Pledge: I have adhered to the Oberlin Honor Code on this assignment.
; Name: Ysabella Atehortua